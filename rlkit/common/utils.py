...
class WeightedMSELoss(nn.Module):
    ...
    def forward(self, inputs, target, weights=None):
        if weights is None:
            return nn.MSELoss(reduction=self.reduction)(inputs, target)

        loss_rows = (inputs - target)**2
...
class SegmentTree(object):
    def __init__(self, capacity, operation, neutral_element):
        ...
        ----------
        capacity: int
            Total size of array--must be power of two
        operation: lambda obj, obj -> obj
            And operation for combining elements (eg. sum, max)
            Must for a mathematical group together with set of
    ...
    def reduce(self, start=0, end=None):
        ...
        to a contiguous subsequence of array.
        ...
        -------
        reduced: obj
            result of reducing self.operation over specified range of array elements
        """
        if end is None:
            end = self._capacity
        if end < 0:
...
class SumSegmentTree(SegmentTree):
    ...
    def find_prefixsum_idx(self, prefixsum):
        ...
        to sample indexes according to the discrete probability
        efficiently.

        Parameters
        ----------
        prefixsum: float
            upperbound on sum of array prefix

        Returns
        -------
        idx: int
            highest index satisfying prefixsum constraint
        """
...
class TBMeanTracker:
    """
    Tensorboard value tracker: allows to batch fixed amt of historical
    values and write their mean into TB

    Designed and tested w/ pytorch-tensorboard in mind
    """
    def __init__(self, writer, batch_size):
        """
        :param writer: writer with close() and add_scalar() methods
        :param batch_size: int size of batch to track
        """
...
class RewardTracker:
    def __init__(self, writer, min_ts_diff=1.0):
        ...
        :param writer: writer to use fo writing stats
        :param min_ts_diff: minimal time diff to track speed
        """
        self.writer = writer
